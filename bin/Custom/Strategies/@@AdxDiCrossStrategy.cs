...
using NinjaTrader.Data;                     // BarsPeriodType / multi-series
...
namespace NinjaTrader.NinjaScript.Strategies
{
    ...
    public class AdxDiCrossStrategy : Strategy
    {
        ...
        public enum DrawdownMode { Ticks, Dollars }
        ...
        // ===== HTF Filters =====
        [NinjaScriptProperty]
        [Display(Name="Use HTF Filters", GroupName="HTF Filters", Order=100)]
        public bool UseHtfFilters { get; set; } = false;

        [NinjaScriptProperty]
        [Display(Name="Use 15m HTF", GroupName="HTF Filters", Order=101)]
        public bool UseHtf15 { get; set; } = true;

        [NinjaScriptProperty, Range(1,int.MaxValue)]
        [Display(Name="15m HTF (minutes)", GroupName="HTF Filters", Order=102)]
        public int Htf15Minutes { get; set; } = 15;

        [NinjaScriptProperty]
        [Display(Name="Use 30m HTF", GroupName="HTF Filters", Order=103)]
        public bool UseHtf30 { get; set; } = true;

        [NinjaScriptProperty, Range(1,int.MaxValue)]
        [Display(Name="30m HTF (minutes)", GroupName="HTF Filters", Order=104)]
        public int Htf30Minutes { get; set; } = 30;

        [NinjaScriptProperty]
        [Display(Name="Use 60m HTF", GroupName="HTF Filters", Order=105)]
        public bool UseHtf60 { get; set; } = true;

        [NinjaScriptProperty, Range(1,int.MaxValue)]
        [Display(Name="60m HTF (minutes)", GroupName="HTF Filters", Order=106)]
        public int Htf60Minutes { get; set; } = 60;

        [NinjaScriptProperty, Range(0.0,100.0)]
        [Display(Name="HTF Min ADX", GroupName="HTF Filters", Order=107)]
        public double HtfMinAdx { get; set; } = 20.0;

        [NinjaScriptProperty]
        [Display(Name="Require HTF Direction Align", GroupName="HTF Filters", Order=108)]
        public bool RequireHtfDirectionAlign { get; set; } = true;

        [NinjaScriptProperty]
        [Display(Name="Use HTF ATR Filter", GroupName="HTF Filters", Order=109)]
        public bool UseHtfAtrFilter { get; set; } = false;

        [NinjaScriptProperty, Range(0.1,double.MaxValue)]
        [Display(Name="Max HTF ATR / LTF ATR", GroupName="HTF Filters", Order=110)]
        public double MaxHtfAtrRatio { get; set; } = 3.0;
        ...
        // HTF indicators
        private ADX adx15, adx30, adx60;
        private DM  dm15, dm30, dm60;
        private ATR atr15, atr30, atr60;
        ...
        private bool HtfFilterAllows(bool isLongSignal, bool isShortSignal)
        {
            if (!UseHtfFilters)
                return true;
            ...
            // 15m
            if (UseHtf15 && BarsArray.Length > 1 && CurrentBars[1] > AdxPeriod + 5)
            {
                anyActive = true;
                allOk &= CheckSingleHtf(adx15, dm15, atr15, isLongSignal, isShortSignal);
            }

            // 30m
            if (UseHtf30 && BarsArray.Length > 2 && CurrentBars[2] > AdxPeriod + 5)
            {
                anyActive = true;
                allOk &= CheckSingleHtf(adx30, dm30, atr30, isLongSignal, isShortSignal);
            }

            // 60m
            if (UseHtf60 && BarsArray.Length > 3 && CurrentBars[3] > AdxPeriod + 5)
            {
                anyActive = true;
                allOk &= CheckSingleHtf(adx60, dm60, atr60, isLongSignal, isShortSignal);
            }

            // If no HTF has enough bars yet, stay flat until HTF context exists
            if (!anyActive)
                return false;
        ...
        private bool CheckSingleHtf(ADX adxHtf, DM dmHtf, ATR atrHtf, bool isLongSignal, bool isShortSignal)
        {
            double adxVal = adxHtf[0];
            if (adxVal < HtfMinAdx)
                return false;

            if (RequireHtfDirectionAlign)
            {
                double diP = dmHtf.DiPlus[0];
                double diM = dmHtf.DiMinus[0];

                if (isLongSignal && !(diP > diM))
                    return false;
                if (isShortSignal && !(diM > diP))
                    return false;
            }

            if (UseHtfAtrFilter)
            {
                double ltfAtrVal = atr[0];
                if (ltfAtrVal.ApproxCompare(0.0) == 0)
                    return false;

                double htfAtrVal = atrHtf[0];
                double ratio     = htfAtrVal / ltfAtrVal;

                if (ratio > MaxHtfAtrRatio)
                    return false;
            }
        ...
        protected override void OnStateChange()
        {
            ...
            {
                // Add higher-timeframe series for HTF filters (always add so parameters
                // can be toggled without recompile; you can disable their use via UseHtfFilters).
                AddDataSeries(BarsPeriodType.Minute, Htf15Minutes);
                AddDataSeries(BarsPeriodType.Minute, Htf30Minutes);
                AddDataSeries(BarsPeriodType.Minute, Htf60Minutes);
            }
            else if (State == State.DataLoaded)
            {
                ...
                // HTF indicators (15 / 30 / 60m)
                if (BarsArray.Length > 1)
                {
                    adx15 = ADX(BarsArray[1], AdxPeriod);
                    dm15  = DM (BarsArray[1], AdxPeriod);
                    atr15 = ATR(BarsArray[1], AtrPeriod);
                }
                if (BarsArray.Length > 2)
                {
                    adx30 = ADX(BarsArray[2], AdxPeriod);
                    dm30  = DM (BarsArray[2], AdxPeriod);
                    atr30 = ATR(BarsArray[2], AtrPeriod);
                }
                if (BarsArray.Length > 3)
                {
                    adx60 = ADX(BarsArray[3], AdxPeriod);
                    dm60  = DM (BarsArray[3], AdxPeriod);
                    atr60 = ATR(BarsArray[3], AtrPeriod);
                }
                ...
                // map legacy ATR multiplier into AtrStopMult if user sets it
        ...
        {
            // Only drive logic from the primary series; HTF indicators update internally
            if (BarsInProgress != 0)
                return;
            ...
            bool longSetup  = allowLong  && adxStrong && crossUp;
            bool shortSetup = allowShort && adxStrong && crossDn;

            bool htfOkLong  = !longSetup  || HtfFilterAllows(true,  false);
            bool htfOkShort = !shortSetup || HtfFilterAllows(false, true);
            ...
            if (Position.MarketPosition == MarketPosition.Flat && windowActive)
            {
                ...
                if (longSetup && htfOkLong)
                    SubmitLongWithCatStops();
                else if (shortSetup && htfOkShort)
