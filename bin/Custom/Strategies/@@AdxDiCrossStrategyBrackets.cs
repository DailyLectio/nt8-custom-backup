...
{
    public class AdxDiCrossStrategy_Brackets : Strategy
    {
        // ===== Enums =====
        ...
        [NinjaScriptProperty] [Display(Name="Use Dyn Target", GroupName="Bracket - Leg1", Order=120)]
        public bool UseDynTarget { get; set; } = true;

        [NinjaScriptProperty, Range(0.5,3.0)] [Display(Name="Dyn ATR Mult", GroupName="Bracket - Leg1", Order=121)]
        public double DynAtrMult { get; set; } = 1.25;

        [NinjaScriptProperty, Range(4,200)] [Display(Name="Dyn Min Ticks", GroupName="Bracket - Leg1", Order=122)]
        public int DynMinTicks { get; set; } = 36;

        [NinjaScriptProperty, Range(4,200)] [Display(Name="Dyn Max Ticks", GroupName="Bracket - Leg1", Order=123)]
        public int DynMaxTicks { get; set; } = 52;

        [NinjaScriptProperty] [Display(Name="Use ADX Boost", GroupName="Bracket - Leg1", Order=124)]
        public bool UseAdxBoost { get; set; } = false;

        [NinjaScriptProperty, Range(10,50)] [Display(Name="ADX Boost Start", GroupName="Bracket - Leg1", Order=125)]
        public int AdxBoostStart { get; set; } = 30;

        [NinjaScriptProperty, Range(1.0,1.5)] [Display(Name="ADX Boost Max Mult", GroupName="Bracket - Leg1", Order=126)]
        ...
        // ===== Failsafe control =====
        [NinjaScriptProperty] [Display(Name="Use FailSafe", GroupName="Risk Control", Order=300)]
        public bool UseFailSafe { get; set; } = true;

        [NinjaScriptProperty, Range(50,10000)] [Display(Name="MaxLossPerTrade ($)", GroupName="Risk Control", Order=301)]
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "AdxDiCrossStrategy_Brackets";
        ...
        // ---- dynamic Leg1 target (ticks) helper
        private int ComputeDynLeg1Ticks()
        {
            // Base on ATR in ticks
            double atrTicks = atr[0] / TickSize;
            double mult = DynAtrMult;

            if (UseAdxBoost && adx[0] > AdxBoostStart)
            {
                // 0..1 ramp over ~10 ADX points above start
                double t = Math.Min(1.0, (adx[0] - AdxBoostStart) / 10.0);
        ...
        protected override void OnBarUpdate()
        {
            ...
            // ---- FailSafe: if enabled and position open, flatten when unrealized <= -MaxLossPerTrade
            if (UseFailSafe && Position.MarketPosition != MarketPosition.Flat)
            {
                ...
                if (unreal <= -Math.Abs(MaxLossPerTrade))
                {
                    ...
                    return; // stop further processing this bar
                }
            }

            if (Position.MarketPosition == MarketPosition.Flat && InWindow())
            {
                // If using dynamic target and Leg1 uses tick mode, we temporarily override Leg1 ticks in the legs array
                int dynTicks = (UseDynTarget && Leg1TargetMode == TargetCalcMode.Ticks) ? ComputeDynLeg1Ticks() : Leg1TargetTicks;

                if (strong && outsideZone)
                {
                    if ((Bias == TradeBias.Both || Bias == TradeBias.LongOnly) && dirLongOk && anchorLongOk && crossUp)
                    {
                        // temporarily push dynamic ticks for Leg1
                        int saved = legs[0].TicksT;
