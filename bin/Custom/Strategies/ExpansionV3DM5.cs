...
{
    public enum V3DExpansionM5DirectionMode
    {
        Both,
        LongOnly,
        ShortOnly
    ...
    public class Expansion_V3D_M5 : Strategy
    {

        // =====================================================================
        // M5 ADDITIONS (Model 5 lab)
        //   Direction filter + per-chart time-block windows.
        //   These are GUARDS layered on top of the existing V3D regime logic;
        //   none of the V3D engine, regime gate, trailing or trade-log paths
        //   are modified.
        // =====================================================================

        [NinjaScriptProperty]
        [Display(Name = "M5 Direction Mode", Description = "Both preserves V3D behavior. LongOnly blocks new shorts. ShortOnly blocks new longs.", GroupName = "0a. M5", Order = 0)]
        public V3DExpansionM5DirectionMode M5DirectionMode { get; set; } = V3DExpansionM5DirectionMode.Both;

        [NinjaScriptProperty]
        [Display(Name = "Enable Time Blocks", Description = "When true, new entries are blocked during the configured HHmm windows below. Position management is never affected.", GroupName = "0a. M5", Order = 1)]
        public bool EnableTimeBlocks { get; set; } = false;

        [NinjaScriptProperty, Range(0, 2359)]
        [Display(Name = "Block 1 Start (HHmm)", Description = "Start of blocked entry window (HHmm). 0 = unused.", GroupName = "0a. M5", Order = 2)]
        public int Block1Start { get; set; } = 0;

        [NinjaScriptProperty, Range(0, 2359)]
        [Display(Name = "Block 1 End (HHmm)", Description = "End of blocked entry window, inclusive. 0 = unused.", GroupName = "0a. M5", Order = 3)]
        public int Block1End { get; set; } = 0;

        [NinjaScriptProperty, Range(0, 2359)]
        [Display(Name = "Block 2 Start (HHmm)", Description = "Second blocked window start. 0 = unused.", GroupName = "0a. M5", Order = 4)]
        public int Block2Start { get; set; } = 0;

        [NinjaScriptProperty, Range(0, 2359)]
        [Display(Name = "Block 2 End (HHmm)", Description = "Second blocked window end. 0 = unused.", GroupName = "0a. M5", Order = 5)]
        public int Block2End { get; set; } = 0;

        [NinjaScriptProperty, Range(0, 2359)]
        [Display(Name = "Block 3 Start (HHmm)", Description = "Third blocked window start. 0 = unused.", GroupName = "0a. M5", Order = 6)]
        public int Block3Start { get; set; } = 0;

        [NinjaScriptProperty, Range(0, 2359)]
        [Display(Name = "Block 3 End (HHmm)", Description = "Third blocked window end. 0 = unused.", GroupName = "0a. M5", Order = 7)]
        ...
        public string AccountNameFilter { get; set; } = "SimM5-A-V3D-EXP-NQ-1A-001";
        ...
        public string ConfiguredStrategyName { get; set; } = "Expansion_V3D_M5";
        ...
        public string TradeLogFolder { get; set; } = @"C:\Users\Valued Customer\NT8_Regimes\5A\TradeLog\V3D";
        // --- Risk ---
        ...
        {
            if (M5DirectionMode == V3DExpansionM5DirectionMode.ShortOnly) return false;
            return IsBaselineMode() || allowLong;
        }

        private bool DirectionShortAllowed()
        {
            if (M5DirectionMode == V3DExpansionM5DirectionMode.LongOnly) return false;
            return IsBaselineMode() || allowShort;
        }

        // M5 time-block helper.  Returns true when the current bar time falls
        // inside one of the up-to-three configured HHmm BLOCK windows.
        private bool IsInBlockedWindow()
        {
            if (!EnableTimeBlocks) return false;
            int hhmm = Time[0].Hour * 100 + Time[0].Minute;
            if (Block1Start > 0 && Block1End > 0 && hhmm >= Block1Start && hhmm <= Block1End) return true;
            if (Block2Start > 0 && Block2End > 0 && hhmm >= Block2Start && hhmm <= Block2End) return true;
            if (Block3Start > 0 && Block3End > 0 && hhmm >= Block3Start && hhmm <= Block3End) return true;
            return false;
        }
...
#pragma warning disable CS0162
        ...
        private void HandleProtectiveExecution(Execution execution)
        {
            ...
            else if (IsExitAction(order.OrderAction) && Position.MarketPosition == MarketPosition.Flat)
            {
                ClearLegState();
            }
        }
        ...
        {
            if (order == null || exitPending || Position.MarketPosition == MarketPosition.Flat) return;
        ...
        {
            double market = Close[0];
            if (Position.MarketPosition == MarketPosition.Long)
                return stopPrice <= RT(market - TickSize);
        ...
        private bool TrySetLeg2Stop(double stopPrice, string reason)
        {
            ...
            if (!IsValidProtectiveStop(stopPrice))
            {
                exitPending = true;
                pendingLeg2StopRearm = false;
                Print(string.Format("[Expansion_V3D-A] Leg2 stop crossed market; exiting runner | Reason:{0} | Stop:{1:F2} | Close:{2:F2}", reason, stopPrice, Close[0]));
        ...
        private void RearmLeg2Stop(string reason)
        {
            ...
            if (Position.MarketPosition == MarketPosition.Long)
            {
                double nextStop = Math.Max(leg2TrailingStop, pivot);
                if (!TrySetLeg2Stop(nextStop, reason)) return;
            }
            else
            {
                double nextStop = leg2TrailingStop == 0.0 ? pivot : Math.Min(leg2TrailingStop, pivot);
                if (!TrySetLeg2Stop(nextStop, reason)) return;
            }

            pendingLeg2StopRearm = false;
            Print(string.Format("[Expansion_V3D-A] Leg2 stop re-armed | Reason:{0} | Stop:{1:F2}", reason, leg2TrailingStop));
        }
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Position.MarketPosition == MarketPosition.Flat)
            {
                ...
                if (greenBrick && DirectionLongAllowed() && !IsInBlockedWindow())
                {
                ...
                else if (redBrick && DirectionShortAllowed() && !IsInBlockedWindow())
                {
                    double stp  = RT(Close[0] + riskTicksI * TickSize);
                    double tgt1 = RT(Close[0] - riskTicksI * TickSize);
