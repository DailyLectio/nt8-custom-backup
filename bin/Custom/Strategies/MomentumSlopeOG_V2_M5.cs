...
{
    public enum MomoOGM5DirectionMode
    {
        Both,
        LongOnly,
        ShortOnly
    ...
    public class MomentumSlopeStrategy_V2_M5 : Strategy
    {

        // =====================================================================
        // M5 ADDITIONS (Model 5 lab)
        //   Direction filter + account-name allow-list + explicit trade-log
        //   folder property. These are GUARDS layered on top of the existing
        //   Trinity / time / slope / anchor / guards machinery; no existing
        //   logic path is altered.
        // =====================================================================

        [NinjaScriptProperty]
        [Display(Name = "M5 Direction Mode", Description = "Both preserves existing behavior. LongOnly blocks new shorts. ShortOnly blocks new longs.", GroupName = "0a. M5", Order = 0)]
        public MomoOGM5DirectionMode M5DirectionMode { get; set; } = MomoOGM5DirectionMode.Both;

        [NinjaScriptProperty]
        [Display(Name = "Account Name Filter", Description = "Exact NT8 account name. Strategy fires only when Account.Name matches. Empty string = no filter (legacy behavior).", GroupName = "0a. M5", Order = 1)]
        public string AccountNameFilter { get; set; } = "";

        [NinjaScriptProperty]
        [Display(Name = "Trade Log Folder", Description = "Folder path where per-account trade logs are written by the external V3D-style exporter. Strategy itself does not write here; the property is exposed so the exporter indicator can read it.", GroupName = "0a. M5", Order = 2)]
        public string TradeLogFolder { get; set; } = @"C:\Users\Valued Customer\NT8_Regimes\5A\TradeLog\1OG";
        ...
        {
            return M5DirectionMode != MomoOGM5DirectionMode.ShortOnly;
        }

        private bool DirectionAllowsShort()
        {
            return M5DirectionMode != MomoOGM5DirectionMode.LongOnly;
        }

        private bool AccountMatchesFilter()
        {
            if (string.IsNullOrWhiteSpace(AccountNameFilter)) return true;
            if (Account == null || string.IsNullOrEmpty(Account.Name)) return false;
            return string.Equals(Account.Name.Trim(), AccountNameFilter.Trim(), StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void PrintDebugEntryBlock(string side, string reason)
        {
            ...
            Print($"{Time[0]} MomentumSlopeOG_V2_M5 debug | account={accountName} symbol={Instrument.FullName} hudSymbol={baseSymbol} side={side} EnableTrinityFilter={EnableTrinityFilter} lane=AllowMomo:{lane} finalRegime={regime} reasonCode={reasonCode} stale={stale} decision=BLOCK reason={reason}");
        }
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "MomentumSlopeStrategy_V2_M5";
        ...
        {
            if (!AccountMatchesFilter())
            {
                PrintDebugEntryBlock("LONG", "m5_account_filter");
                return;
            }
            if (!DirectionAllowsLong())
            {
                PrintDebugEntryBlock("LONG", "m5_direction");
                return;
            }
        ...
        private void SubmitShortWithStops()
        {
            if (!AccountMatchesFilter())
            {
                PrintDebugEntryBlock("SHORT", "m5_account_filter");
                return;
            }
            if (!DirectionAllowsShort())
            {
                PrintDebugEntryBlock("SHORT", "m5_direction");
                return;
            }
            if (!IsBotAllowedByTrinity())
            {
