// CC BY-NC 4.0 — Scalp HUD (Futures) — NT8 v1.3.3
// Fixes: SimpleFont namespace, limit HUD positions to TextPosition supported on your NT8 build.
// Internal session VWAP (no Order Flow+). ES/NQ presets, EMA band, cooldown, ADX percentile,
// round-number filter, right-edge Debug HUD.
...
{
    public class ScalpHUD_Futures_NT8_v1_3_3 : Strategy
    {
        // ===== Orders / Presets =====
        [NinjaScriptProperty, Display(Name="Contracts", Order=0, GroupName="Orders")]
        public int Contracts { get; set; } = 1;

        [NinjaScriptProperty, Display(Name="Risk:Reward", Order=1, GroupName="Orders", Description="1.0 = 1:1, 1.5 = 1:1.5")]
        ...
        // ===== Debug HUD =====
        // Limited to TextPosition members available on your NT8 build.
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "Scalp HUD (Futures) — NT8 v1.3.3";
                Description = "Scalp HUD logic w/ ES/NQ presets, filters, debug HUD, internal VWAP.";
        ...
        private void SetStopsAndTargets(bool longEntry, double entryPrice)
        {
            ...
            if (longEntry)
            {
                if (!double.IsNaN(lastSigLow))
                    stopDistance = entryPrice - lastSigLow;
            }
            else
            {
                if (!double.IsNaN(lastSigHigh))
                    stopDistance = lastSigHigh - entryPrice;
            }

            if (stopDistance <= TickSize || double.IsNaN(stopDistance))
                stopDistance = Math.Max(atr14[0] * 0.5, 2 * TickSize);
