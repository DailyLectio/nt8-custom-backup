// CC BY-NC 4.0
// ============================================================================
// Expansionv4 — LIVE clone of V3_Expansion_Rider_V3C
...
{
    public enum V4ExpansionTradeDirectionMode
    {
        LongAndShort,
        LongOnly,
        ShortOnly
    ...
    public enum V4ExpansionTrailMode
    {
        Off,
        BarNTrailing,
        AtrStep
    ...
    public enum V4ExpansionWobbleMode
    {
    ...
    public class Expansionv4 : Strategy
    {
        ...
        public V4ExpansionTradeDirectionMode TradeDirection { get; set; } = V4ExpansionTradeDirectionMode.LongOnly;

        [NinjaScriptProperty, Range(0, 100)]
        [Display(Name="Min HUD Confidence", Description="0 disables this gate. Uses the V3C HUD confidence field when present.", GroupName="2c. Research Gates", Order=0)]
        public int MinConfidenceThreshold { get; set; } = 0;

        [NinjaScriptProperty]
        [Display(Name="Wobble Handler", Description="Default B: close Leg1 and keep Leg2 protected. A/C are for SIM comparison.", GroupName="2c. Research Gates", Order=1)]
        public V4ExpansionWobbleMode WobbleHandler { get; set; } = V4ExpansionWobbleMode.Leg1Close_Leg2Runs_Protected;
        ...
        public int Block1Start { get; set; } = 1000;
        ...
        [NinjaScriptProperty, Range(0, 100)]
        [Display(Name="Max Trades Per Day", Description="Caps new trade cycles per session. 0 = OFF. NQ research default 14; ES override 3.", GroupName="5. Same-Direction Cap", Order=1)]
        ...
        [Display(Name="Daily Loss Circuit ($)", Description="Research/SIM default. Apex 50K 2C MNQ preset: 750.", GroupName="6. Consecutive Loss Limit", Order=1)]
        public double DailyLossCircuit { get; set; } = 2650.0;

        [NinjaScriptProperty, Range(0.0, double.MaxValue)]
        [Display(Name="Apex 50K Daily Loss Preset ($)", Description="Reference only; set Daily Loss Circuit to this value for Apex 50K 2C MNQ.", GroupName="6. Consecutive Loss Limit", Order=2)]
        public double Apex50KDailyLossCircuitPreset { get; set; } = 750.0;
        ...
        public V4ExpansionTrailMode Leg1TrailMode { get; set; } = V4ExpansionTrailMode.Off;
        ...
        public V4ExpansionTrailMode Leg2TrailMode { get; set; } = V4ExpansionTrailMode.Off;
        ...
        private double _sessionStartCumProfit = 0.0;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description                                 = "Expansionv4: V4 parked clone from VC3 Expansion live lineage with researched gates and stop-protection hooks.";
                Name                                        = "Expansionv4";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                _sessionStartCumProfit = _cycleStartCumProfit;
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Position.MarketPosition == MarketPosition.Flat)
            {
                ...
                bool lossBlocked = ConsecutiveLossBlocked() || DailyLossCircuitBlocked() || MaxTradesPerDayBlocked();
            ...
            if (Position.MarketPosition != MarketPosition.Flat)
            {
                ...
                {
                    ExitWobbleExpansionLegs("Wobble Eject");
                    awaitingLeg2 = false;
                    return;
                }

                if (!leg1Hit && Leg1TrailMode != V4ExpansionTrailMode.Off)
                ...
                if (!awaitingLeg2 && Leg2TrailMode != V4ExpansionTrailMode.Off)
                ...
                if (leg1Hit && Leg2TrailMode == V4ExpansionTrailMode.Off)
                {
        ...
        private void ExitWobbleExpansionLegs(string exitSignal)
        {
            int qty = Position.Quantity;
            if (qty <= 0 || Position.MarketPosition == MarketPosition.Flat)
                return;

            if (WobbleHandler == V4ExpansionWobbleMode.FlattenBoth)
            {
                ExitOpenExpansionLegs(exitSignal);
                return;
            }
            ...
            if (WobbleHandler == V4ExpansionWobbleMode.Leg2Close_Leg1Runs_Protected)
            {
                if (Position.MarketPosition == MarketPosition.Long)
                    ExitLong(legQty, exitSignal, "Leg2");
                else if (Position.MarketPosition == MarketPosition.Short)
                    ExitShort(legQty, exitSignal, "Leg2");

                ReArmSurvivorStop("Leg1");
                return;
            }

            if (Position.MarketPosition == MarketPosition.Long)
                ExitLong(legQty, exitSignal, "Leg1");
            else if (Position.MarketPosition == MarketPosition.Short)
                ExitShort(legQty, exitSignal, "Leg1");

            leg1Hit = true;
            ReArmSurvivorStop("Leg2");
        }

        private void ReArmSurvivorStop(string survivorSignal)
        {
            if (Position.MarketPosition == MarketPosition.Flat)
                return;
            ...
            if (string.Equals(survivorSignal, "Leg1", StringComparison.OrdinalIgnoreCase))
                leg1ManagedStop = pivot;
            else
                leg2TrailingStop = pivot;

            SetStopSafe(survivorSignal, pivot, isLong);
            DebugGate("Re-armed survivor stop for " + survivorSignal + " at " + pivot);
        }
        ...
        private bool IsExpansionAllowed(out bool allowLong, out bool allowShort)
        {
            ...
            {
                double confidence = GetHudConfidence(hud);
                if (confidence < MinConfidenceThreshold)
                {
                    DebugGate("Blocked: confidence=" + confidence + " < " + MinConfidenceThreshold);
                    return false;
                }
            }
        ...
        private double GetHudConfidence(object hud)
        {
            if (hud == null)
                return 0.0;

            string[] names = new string[] { "Confidence", "RegimeConfidence", "ConfidenceScore", "MinConfidence", "HudConfidence" };
            Type t = hud.GetType();
            ...
            foreach (string name in names)
            {
                ...
                object value = p.GetValue(hud, null);
                if (value == null)
                    continue;
        ...
        private void ApplyLegTrail(string signal, V4ExpansionTrailMode mode, int nBars, int offsetTicks,
            double step1Atr, double step2Atr, int bePlusTicks, double trailAtrMult,
            double entryPrice, ref double managedStop)
        {
            if (mode == V4ExpansionTrailMode.Off || tradeDir == 0 || entryPrice <= 0)
                return;
            ...
            switch (mode)
            {
                case V4ExpansionTrailMode.BarNTrailing:
                ...
                case V4ExpansionTrailMode.AtrStep:
                {
        ...
        {
            if (TradeDirection == V4ExpansionTradeDirectionMode.LongOnly)
                allowShort = false;
            else if (TradeDirection == V4ExpansionTradeDirectionMode.ShortOnly)
                allowLong = false;
        }
        ...
        private bool DailyLossCircuitBlocked()
        {
            if (DailyLossCircuit <= 0)
                return false;

            double sessionPnL = SystemPerformance.AllTrades.TradesPerformance.Currency.CumProfit - _sessionStartCumProfit;
            bool blocked = sessionPnL <= -DailyLossCircuit;
            if (blocked)
                DebugGate("Blocked: daily loss circuit reached. SessionPnL=" + sessionPnL.ToString("F2"));
            return blocked;
        }

        private bool MaxTradesPerDayBlocked()
        {
