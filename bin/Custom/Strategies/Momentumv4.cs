...
{
    public class Momentumv4 : Strategy
    {
        ...
        [NinjaScriptProperty]
        [Display(Name="Enable HMM TrendDown Block", GroupName="V4 Research Gates", Order=1)]
        public bool EnableHMMTrendDownBlock { get; set; } = true;

        [NinjaScriptProperty]
        [Display(Name="HMM Blocked States", GroupName="V4 Research Gates", Order=2)]
        public string HMMBlockedStates { get; set; } = "TrendDown";

        [NinjaScriptProperty, Range(0, 100)]
        [Display(Name="Min HUD Confidence", GroupName="V4 Research Gates", Order=3)]
        public int MinConfidenceThreshold { get; set; } = 0;

        [NinjaScriptProperty, Range(0, 20)]
        [Display(Name="Max Same-Direction Trades", GroupName="V4 Research Gates", Order=4)]
        public int MaxSameDirTrades { get; set; } = 2;

        [NinjaScriptProperty, Range(0, 100)]
        [Display(Name="Max Trades Per Day", GroupName="V4 Research Gates", Order=5)]
        ...
        private bool IsV4HudAllowed()
        {
            string baseSymbol;
            Indicators.RegimeMatrixHUD_V3D hud;
            if (!TryGetTrinityState(out baseSymbol, out hud) || hud == null)
                return !EnableTrinityFilter;

            if (hud.StaleDataFlag)
                return false;

            if (EnableTrinityFilter && !hud.IsMomoAllowed)
                return false;

            if (EnableHMMTrendDownBlock && IsHudHmmBlocked(hud))
                return false;

            if (MinConfidenceThreshold > 0 && GetHudConfidence(hud) < MinConfidenceThreshold)
                return false;

            return true;
        }

        private bool IsHudHmmBlocked(object hud)
        {
            if (hud == null || string.IsNullOrEmpty(HMMBlockedStates))
                return false;

            string state = GetHudString(hud, "HMMMicro");
            ...
            foreach (string blocked in HMMBlockedStates.Split(','))
                if (string.Equals(blocked.Trim(), state.Trim(), StringComparison.OrdinalIgnoreCase))
                    return true;

            return false;
        }

        private string GetHudString(object hud, string propertyName)
        {
            System.Reflection.PropertyInfo p = hud.GetType().GetProperty(propertyName);
            object value = p == null ? null : p.GetValue(hud, null);
        ...
        private double GetHudConfidence(object hud)
        {
            ...
            {
                System.Reflection.PropertyInfo p = hud.GetType().GetProperty(name);
                if (p == null)
                    continue;
        ...
        private void PrintDebugEntryBlock(string side, string reason)
        {
            ...
            Print($"{Time[0]} Momentumv4 debug | account={accountName} symbol={Instrument.FullName} hudSymbol={baseSymbol} side={side} EnableTrinityFilter={EnableTrinityFilter} lane=AllowMomo:{lane} finalRegime={regime} reasonCode={reasonCode} stale={stale} decision=BLOCK reason={reason}");
        }
        ...
        public double DailyLossLimit { get; set; } = 700.0;

        [NinjaScriptProperty, Range(0.0, double.MaxValue)]
        [Display(Name="Apex 50K Daily Loss Preset ($)", Description="Reference only; set Loss Limit to this value for Apex 50K 2C MNQ.", GroupName="Guards", Order=8)]
        public double Apex50KDailyLossPreset { get; set; } = 250.0;
        ...
        public int EndTime1 { get; set; } = 100000;    // 10:00:00

        [NinjaScriptProperty]
        [Display(Name="Start Time 2 (HHmmss)", GroupName="Time Filters", Order=3)]
        public int StartTime2 { get; set; } = 103000;

        [NinjaScriptProperty]
        [Display(Name="End Time 2 (HHmmss)", GroupName="Time Filters", Order=4)]
        public int EndTime2 { get; set; } = 111500;

        [NinjaScriptProperty]
        [Display(Name="Start Time 3 (HHmmss)", GroupName="Time Filters", Order=5)]
        public int StartTime3 { get; set; } = 120000;

        [NinjaScriptProperty]
        [Display(Name="End Time 3 (HHmmss)", GroupName="Time Filters", Order=6)]
        public int EndTime3 { get; set; } = 154500;

        [NinjaScriptProperty]
        [Display(Name="Block Open", GroupName="Time Filters", Order=7)]
        public bool BlockOpen { get; set; } = false;

        [NinjaScriptProperty]
        [Display(Name="Open Block Start (HHmmss)", GroupName="Time Filters", Order=8)]
        public int OpenBlockStart { get; set; } = 93000;  // 09:30:00

        [NinjaScriptProperty]
        [Display(Name="Open Block End (HHmmss)", GroupName="Time Filters", Order=9)]
        public int OpenBlockEnd { get; set; } = 93900;   // 09:39:00

        [NinjaScriptProperty]
        [Display(Name="Block News", GroupName="Time Filters", Order=10)]
        public bool BlockNews { get; set; } = false;

        [NinjaScriptProperty]
        [Display(Name="News Block Start (HHmmss)", GroupName="Time Filters", Order=11)]
        public int NewsBlockStart { get; set; } = 95600; // 09:56:00

        [NinjaScriptProperty]
        [Display(Name="News Block End (HHmmss)", GroupName="Time Filters", Order=12)]
        public int NewsBlockEnd { get; set; } = 100300;   // 10:03:00

        [NinjaScriptProperty]
        [Display(Name="Block Lunch", GroupName="Time Filters", Order=13)]
        public bool BlockLunch { get; set; } = false;

        [NinjaScriptProperty]
        [Display(Name="Lunch Block Start (HHmmss)", GroupName="Time Filters", Order=14)]
        public int LunchBlockStart { get; set; } = 113000; // 11:30:00

        [NinjaScriptProperty]
        [Display(Name="Lunch Block End (HHmmss)", GroupName="Time Filters", Order=15)]
        public int LunchBlockEnd { get; set; } = 132000;   // 13:20:00

        [NinjaScriptProperty]
        [Display(Name="Flatten Near End", GroupName="Time Filters", Order=16)]
        public bool FlattenNearEnd { get; set; } = false;

        [NinjaScriptProperty, Range(0, 1800)]
        [Display(Name="Flatten Seconds Before End", GroupName="Time Filters", Order=17)]
        ...
        private bool IsInAllowedWindow()
        {
            ...
            bool inMain = InTimeRange(t, StartTime1, EndTime1)
                || InTimeRange(t, StartTime2, EndTime2)
                || InTimeRange(t, StartTime3, EndTime3);
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (CurrentBar < 2)
            {
                if (Bars.IsFirstBarOfSession)
                {
                    ...
                    if (ResetOnNewSession)
                    {
                        ...
                        sameDirCount = 0; lastEntryDir = 0; tradesToday = 0;
                    }
                }
            ...
            bool canEnter = !tradingHalted && IsV4HudAllowed() && !MaxTradesPerDayBlocked();
            ...
            if (timeOk && canEnter && Position.MarketPosition == MarketPosition.Flat && slopeEntryOk && adxOk && ciOk)
            {
                ...
                bool doShort = LongsOnly ? false : (UseDiCrossEntry ? crossDown : (SideAnchor == AnchorMode.None ? true : Close[0] <= anchor));
        ...
        {
            return MaxSameDirTrades > 0 && dir == lastEntryDir && sameDirCount >= MaxSameDirTrades;
        }

        private bool MaxTradesPerDayBlocked()
        {
            return MaxTradesPerDay > 0 && tradesToday >= MaxTradesPerDay;
        }

        private void RegisterSameDir(int dir)
        {
            if (dir == lastEntryDir)
                sameDirCount++;
            else
