...
{
    // ============= AdxDiCrossBracketOG =============
    // - Time-window aware entries (ETH + RTH) with toggles
    // - ATR stop + RR target at entry
    // - Bar-N trailing (switch after N)
    // - EMA trailing (switch after N)
    // - ATR Step (BE+ then ATR trail)
    // - Optional entry quality filters (ADX slope, DI gap)
    public class AdxDiCrossBracketOG_V2 : Strategy
    {
        // ===== Trinity / Regime HUD Gate =====
        [NinjaScriptProperty]
        [Display(Name="1. Enable Trinity Filter", Description="If true, listens to the Regime HUD for ADX permission.", GroupName="Trinity Command Center", Order=0)]
        ...
        private bool IsBotAllowedByTrinity()
        {
            ...
            if (Indicators.RegimeMatrixHUD_V3D.InstancesV3D.TryGetValue(baseSymbol, out hudInstance) && hudInstance != null)
                return hudInstance.IsADX_DIAllowed;
        ...
        private const string Stage1BotName = "OG_AdxDiCrossBracketOG";
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Position.MarketPosition == MarketPosition.Flat)
            {
                ...
                if (timeOK && adxStrong && crossUp && AdxSlopeOK() && DIGapOK_Long())
                {
                    SubmitLongWithCatStops();
                }
                else if (timeOK && adxStrong && crossDn && AdxSlopeOK() && DIGapOK_Short())
                {
