...
using NinjaTrader.Gui.Tools;                 // [Range] attribute
using NinjaTrader.NinjaScript;
using NinjaTrader.NinjaScript.Strategies;
using NinjaTrader.NinjaScript.Indicators;   // OrderFlowVWAP, VWAPResolution, VWAPStandardDeviations
#endregion

// EMA crossover entries + VWAP/Trail exits + up to TWO higher-TF confirmations.
// Exit VWAP source selectable: Custom Session VWAP (internal calc) or Order Flow VWAP (OF+).
// Run on a Heiken-Ashi chart for parity with your TV tests.
// Analyzer: Order fill resolution = High / 1-tick for intrabar realism.
...
// EMA crossover entries + trailing stop + selectable VWAP exit source (Order Flow VWAP).
// New in v1.1: option to drive OF VWAP from the underlying (non-HA) series; daily kill switch; two TF confirmations.
// Run on a Heiken-Ashi chart. Analyzer: Order fill resolution = High / 1-tick.

namespace NinjaTrader.NinjaScript.Strategies
{
    public class EMA_VWAP_OF_v1_1 : Strategy
    {
        // -------------------
        // Inputs (Parameters)
        // -------------------
        ...
        // ---- Order Flow VWAP settings (from your NT Help signature) ----
        [NinjaScriptProperty]
        ...
        // ---- Underlying input option for VWAP ----
        [NinjaScriptProperty]
        ...
        // ---- Trading hours / risk ----
        [NinjaScriptProperty]
        [Display(Name = "Only RTE (0930-1600 ET)", GroupName = "Trading Hours", Order = 1)]
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.DataLoaded)
            {
                ...
                // Instantiate OF VWAP; choose input based on underlying flag
                ...
                // OrderFlowVWAP(ISeries<double> input, VWAPResolution resolution, TradingHours tradingHoursInstance,
                //               VWAPStandardDeviations numStandardDeviations, double sD1Multiplier, double sD2Multiplier, double sD3Multiplier)
        ...
        protected override void OnBarUpdate()
        {
            ...
            // Exits via Order Flow VWAP + trailing stop
            if (UseVWAPExit && ofVwapSeries != null)
            {
                double exitVwap = ofVwapSeries[0];
