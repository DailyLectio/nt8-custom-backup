...
{
    public class MomentumSlopeHybridScalp_v2 : Strategy
    {
        // ===== Enums =====
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "MomentumSlopeHybridScalp_v2";
                ...
                CiEntryThreshold = 60; AdxEntryThreshold = 18; // Tightened from 18 to reduce noise
                UseDiCrossEntry = true;
                
                // --- Risk & StopX ---
                AtrStopMult = 0.75; 
                AtrStopLen = 14; 
                MaxStopTicks = 25; // <--- NEW: Hard Cap on Stop Loss (~6 points)
                
                UseExitOnDiCross = true;    // <--- NEW: Kill if trend reverses
                UseExitOnAdxHook = true;    // <--- NEW: Kill if momentum dies

                // Leg 1: The Banker (2 Contracts)
                Qty1 = 2;
                Target1Ticks = 40; 
                Trail1Mode = TrailMode.TickTrail;
                Trail1TickTrailTicks = 20; // Loosened slightly for 1m candle noise
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Position.MarketPosition != MarketPosition.Flat)
            {
                ...
                // Kill if ADX hooks down (momentum failure)
                if (UseExitOnAdxHook && CurrentBar > 1)
                {
                    bool adxFalling = adx[0] < adx[1] && adx[1] < adx[2]; // 2 bars of falling ADX
                    if (adxFalling)
                    {
            ...
            if (Position.MarketPosition == MarketPosition.Flat && !tradingHalted)
            {
                ...
                if (adxOk && ciOk)
                {
                    // Dynamic stop with Max Clamp
                    double atrValue = atrStop[0];
                    int calcStop = (int)Math.Round((atrValue * AtrStopMult) / TickSize);
                    int finalStop = Math.Min(MaxStopTicks, Math.Max(5, calcStop)); // Clamp between 5 and MaxTicks

                    if (UseDiCrossEntry && crossUp) SubmitSplitEntry(true, finalStop);
                    else if (UseDiCrossEntry && crossDown) SubmitSplitEntry(false, finalStop);
                }
            }
        }
        ...
        // Legs (Simplified for brevity - keep your existing leg params here)
