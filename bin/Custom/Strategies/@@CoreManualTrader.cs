...
{
    public class CoreManualTrader : Strategy
    {
        ...
        [NinjaScriptProperty, Display(Name = "Entry From", GroupName = "Entry", Order = 0)]
        public EntrySource EntryFrom { get; set; } = EntrySource.Price;

        [NinjaScriptProperty, Display(Name = "Latch After Arm", GroupName = "Entry", Order = 1)]
        public bool LatchAfterArm { get; set; } = false;

        [NinjaScriptProperty, Display(Name = "Confirm Any Time", GroupName = "Entry", Order = 2)]
        public bool ConfirmAnyTime { get; set; } = false;

        [NinjaScriptProperty, Display(Name = "Auto Direction from Targets", GroupName = "Entry", Order = 3)]
        public bool AutoDirection { get; set; } = true;

        [NinjaScriptProperty, Display(Name = "Trade Bias", GroupName = "Entry", Order = 4)]
        public TradeBias Bias { get; set; } = TradeBias.Both;

        [NinjaScriptProperty, Display(Name = "Order Type", GroupName = "Entry", Order = 5)]
        public OrderPref OrderType { get; set; } = OrderPref.Market;

        [NinjaScriptProperty, Display(Name = "Entry Level Price", GroupName = "Levels", Order = 0)]
        public double EntryLevel { get; set; } = 0;

        [NinjaScriptProperty, Display(Name = "Start Level (if Entry From = Level)", GroupName = "Levels", Order = 1)]
        public LevelName StartLevel { get; set; } = LevelName.POC;

        [NinjaScriptProperty, Display(Name = "Use Playbook Preset", GroupName = "Playbook", Order = 0)]
        public bool UsePlaybookPreset { get; set; } = false;

        [NinjaScriptProperty, Display(Name = "Preset", GroupName = "Playbook", Order = 1)]
        public PlaybookPreset Preset { get; set; } = PlaybookPreset.None;

        [NinjaScriptProperty, Range(0, int.MaxValue), Display(Name = "PB T1 Qty (0=off)", GroupName = "Playbook", Order = 2)]
        public int PBTarget1Qty { get; set; } = 1;

        [NinjaScriptProperty, Range(0, int.MaxValue), Display(Name = "PB T2 Qty (0=off)", GroupName = "Playbook", Order = 3)]
        public int PBTarget2Qty { get; set; } = 1;
        ...
        private readonly Dictionary<string, double> legLimitPrice = new Dictionary<string, double>();
        private readonly Dictionary<string, bool> legIsLong = new Dictionary<string, bool>();
        private readonly HashSet<string> legNeedsResub = new HashSet<string>();
        ...
        {
            if (EntryFrom == EntrySource.Level)
            {
                double lvl = LevelOf(StartLevel);
                if (lvl.ApproxCompare(0.0) != 0)
                    return lvl;
            }
            return EntryLevel;
        }
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (EntryMode == EntryTriggerMode.ManualLimit)
            {
                ...
                if (legNeedsResub.Count > 0)
                {
                    foreach (var sig in new List<string>(legNeedsResub))
                    {
                        ...
                        double px = RT(legLimitPrice[sig]);
                        bool isLong = legIsLong[sig];
                        int qty = TargetQtyForSignal(sig);
                        if (qty <= 0)
                        {
                            legNeedsResub.Remove(sig);
                            continue;
                        ...
                        if (isLong) EnterLongLimit(qty, px, sig);
                        else EnterShortLimit(qty, px, sig);
            ...
            // Confirm-next-open mode (with latch / confirm-any-time)
            double ep = GetStartPrice();

            if (!setupArmed)
            {
                if ((Bias == TradeBias.Both || Bias == TradeBias.LongOnly) &&
                    Close[0] > ep && Close[1] <= ep)
                {
                    setupArmed = true;
                    longSetup = true;
                    shortSetup = false;
                }
                else if ((Bias == TradeBias.Both || Bias == TradeBias.ShortOnly) &&
                         Close[0] < ep && Close[1] >= ep)
                {
                    setupArmed = true;
                    shortSetup = true;
            ...
            if (longSetup)
            {
                if (ConfirmAnyTime || Open[0] > ep)
                {
                    UpdateButton("Enter LONG", true);
                }
                else if (!LatchAfterArm)
                {
                    setupArmed = false;
                    longSetup = shortSetup = false;
                    UpdateButton("Enter (waiting…)", false);
                ...
                {
                    UpdateButton("Enter LONG (latched)", false);
                }
            }
            else if (shortSetup)
            {
                ...
                {
                    UpdateButton("Enter SHORT (latched)", false);
                }
            }
        }
        ...
        private void UpdateManualLimitButton()
        {
            ...
            bool longIntent =
                (Bias == TradeBias.LongOnly) ||
                (Bias == TradeBias.Both && ep < Close[0]);

            bool shortIntent =
                (Bias == TradeBias.ShortOnly) ||
                (Bias == TradeBias.Both && ep > Close[0]);

            string label;
            if (longIntent && !shortIntent) label = "Place BUY LIMIT";
            else if (shortIntent && !longIntent) label = "Place SELL LIMIT";
        ...
        private void TryEnterManualLimit()
        {
            ...
            if (longIntent)
                SubmitBrackets(true, ep, true);
            if (shortIntent)
                SubmitBrackets(false, ep, true);
        ...
        private void ApplyPlaybookIfAny()
        {
            if (!UsePlaybookPreset || Preset == PlaybookPreset.None)
                return;

            Target1Qty = Target2Qty = Target3Qty = Target4Qty = 0;

            switch (Preset)
            {
                case PlaybookPreset.B2_to_B4:
                    EntryFrom = EntrySource.Level;
                    StartLevel = LevelName.B2;
                    Target1 = LevelName.B4;
                    Target1Qty = PBTarget1Qty;
                    break;

                case PlaybookPreset.R2_to_R4:
                    EntryFrom = EntrySource.Level;
                    StartLevel = LevelName.R2;
                    Target1 = LevelName.R4;
                    Target1Qty = PBTarget1Qty;
                    break;

                case PlaybookPreset.B2_to_POC_to_R2:
                    EntryFrom = EntrySource.Level;
                    StartLevel = LevelName.B2;
                    Target1 = LevelName.POC;
                    Target2 = LevelName.R2;
                    Target1Qty = PBTarget1Qty;
                    Target2Qty = PBTarget2Qty;
                    break;

                case PlaybookPreset.R2_to_POC_to_B2:
                    EntryFrom = EntrySource.Level;
                    StartLevel = LevelName.R2;
                    Target1 = LevelName.POC;
                    Target2 = LevelName.B2;
                    Target1Qty = PBTarget1Qty;
                    Target2Qty = PBTarget2Qty;
                    break;

                case PlaybookPreset.B2_to_R2:
                    EntryFrom = EntrySource.Level;
                    StartLevel = LevelName.B2;
                    Target1 = LevelName.R2;
                    Target1Qty = PBTarget1Qty;
                    break;

                case PlaybookPreset.R2_to_B2:
                    EntryFrom = EntrySource.Level;
                    StartLevel = LevelName.R2;
                    Target1 = LevelName.B2;
                    Target1Qty = PBTarget1Qty;
                    break;
            }
            ...
            if (AutoDirection)
            {
                double s = LevelOf(StartLevel);
                double t1 = LevelOf(Target1);
                if (s.ApproxCompare(0.0) != 0 && t1.ApproxCompare(0.0) != 0)
                {
                    if (t1 > s) Bias = TradeBias.LongOnly;
                    else if (t1 < s) Bias = TradeBias.ShortOnly;
                }
            }
        }
		    ...
		    string nativeError)
		{
		    ...
		    // Only manage our four leg signals
		    if (order.Name != "L1" && order.Name != "L2" && order.Name != "L3" && order.Name != "L4")
		        return;
		
		    // Track last working limit price (also updates when you drag the order on the chart)
		    if (order.OrderType == NinjaTrader.Cbi.OrderType.Limit
		        && orderState == NinjaTrader.Cbi.OrderState.Working)
		    {
		    ...
		    if (orderState == NinjaTrader.Cbi.OrderState.Filled)
		    {
		        ...
		        if (legLimitPrice.ContainsKey(order.Name))
		            legLimitPrice.Remove(order.Name);
		
		        if (legIsLong.ContainsKey(order.Name))
		            legIsLong.Remove(order.Name);
		    }
		}
    }
}
